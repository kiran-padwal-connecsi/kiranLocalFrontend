...
def inbox(message_id):
    ...
    try:
        ...
        message_id_list=[]
        for item in data['data']:
            to_email_id = item['to_email_id']
            to_email_id = str(to_email_id).lower()
            print('small letter = ',to_email_id)
            if to_email_id == email_id:
               inboxList.append(item)
               message_id_list.append(item['message_id'])
        # print(mylist)
        for item in conv_data['data']:
            to_email_id = item['to_email_id']
            to_email_id=to_email_id.lower()
            if to_email_id == email_id and item['message_id'] not in message_id_list:
               inboxList.append(item)
